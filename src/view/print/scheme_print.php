<div id="dontshow">
    <div class="row" id="form_print">
        <div class="col-lg-12">
            <div class="headerprint">
                <img src="src/assets/images/LogoCSAV.png" alt="logo" class="img-25" width="100px">
                <div class="headertext">
                    <h6 class=" fw-bold mb-0">SCHOLARLYSYNC</h6>
                    <div class="text-uppercase xsd">School Based Web
                        Access Scholars Information <br>Tracking System with Email Notifications
                    </div>
                </div>
            </div>
        </div>
         <div class="text-center">
            <span class="prtitle">Scholarhsip List</span><br>
            <small>{{dnow | date: 'MMM dd, yyyy'}}</small>
        </div>
        <div class="col-lg-12 sec1">
            <table class="table table-bordered ">
                <thead>
                    <tr>
                        <th class="fw-bold" width="8%">No.</th>
                        <th class="fw-bold">Name</th>
                        <th class="fw-bold">Type</th>
                        <th class="fw-bold">Assistance</th>
                        <th class="fw-bold">Deadline</th>
                        <th class="fw-bold">Created By</th>
                        <th class="fw-bold text-center" width="1%" nowrap>Status</th>
                    </tr>
                </thead>
                <tbody>
                    <tr ng-repeat="sch in schlist">
                        <td>{{sch.schid}}</td>
                        <td>{{sch.schname}}</td>
                        <td>{{sch.typename}}</td>
                        <td>{{sch.awardamnt | number: 2}}</td>
                        <td>{{sch.awardate | date:'MMM dd, yyyy'}}</td>
                        <td>{{sch.shortName}}</td>
                        <td class="text-center">
                            {{sch.schstatus == 1? 'Active' : 'Closed'}}
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>