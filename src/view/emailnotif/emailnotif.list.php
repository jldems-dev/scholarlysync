<h1 class="mt-4">Email Notifications List</h1>
<ol class="breadcrumb mb-4">
    <li class="breadcrumb-item active">Manage Email Notifications List</li>
</ol>
<div class="row">
    <div class="col-lg-12 col-sm-12">
        <div class="filter">
            <div class="d-flex align-items-center justify-content-end gap-2">
                <button type="button" class="btn btn-sms-primary" ng-click="email_add()" >Send Concern</button>
                <button type="button" class="btn btn-sms-dark" ng-click="print('form_print', 'Student List');">Print</button>
            </div>
        </div>
    </div>
    <div class="col-lg-12 col-sm-12">
        <div class="filter">
            <div class="row">
                <div class="col-lg-5 col-sm-12 col-md-12">
                    <div class="sms-form">
                        <i class="fa fa-search"></i>
                        <input type="text" class="form-input form-input-search" placeholder="Search" ng-model="search">
                    </div>
                </div>
                <div class="col-lg-7 col-sm-12 col-md-12">
                    <div class="d-flex align-items-center justify-content-end gap-2">
                        <div class="d-flex align-items-center filter-fields">
                            <select ng-model="date" class="form-input w-100">
                                <option ng-value="-1">Date As Of</option>
                                <option ng-value="0">Today</option>
                                <option ng-value="1">This Week</option>
                                <option ng-value="2">This Month</option>
                                <option ng-value="3">This Year</option>
                            </select>
                        </div>
                        <button class="btn btn-md btn-sms-primary" ng-click="email_filter(date)"><i class="fa-solid fa-sliders"></i></button>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-lg-12">
        <div class="table-css table-responsive">
            <table class="table table-bordered ">
                <thead>
                    <tr>
                        <th class="fw-bold" width="8%" nowrap>Email No </th>
                        <th class="fw-bold" nowrap>From</th>
                        <th class="fw-bold" nowrap>To</th>
                        <th class="fw-bold" nowrap>Subject</th>
                        <th class="fw-bold" nowrap>Date</th>
                        <th class="fw-bold text-center" nowrap>Email Status</th>
                        <th class="fw-bold text-center" ng-hide="userid.UserTypeRID == 2" nowrap>Option</th>
                    </tr>
                </thead>
                <tbody>
                    <tr ng-repeat="eml in filteredi = (emaillist | filter: search) | limitTo:items_per_page:items_per_page*(current_page-1)">
                        <td>{{eml.enid}}</td>
                        <td>{{eml.fromemail}}</td>
                        <td>{{eml.email}}</td>
                        <td>{{eml.subjects}}</td>
                        <td>{{eml.sentdate | date: 'MMM dd yyyy'}}</td>
                        <td class="text-center">{{eml.emailstatus == 0? 'Draft' : eml.emailstatus == 1 ? 'Sent' : 'Received'}}</td>
                        <td width="8%" ng-hide="userid.UserTypeRID == 2">
                            <div class="d-flex align-items-center justify-content-center gap-3">
                                <a type="button" ng-click="state_go(eml.enid, 0)" ng-hide="eml.emailstatus == 1 || eml.emailstatus == 2"><i class="fa-light fa-pen-to-square text-success"></i></a>
                                <a type="button" ng-click="state_go(eml.enid, 1)" ng-show="eml.emailstatus == 1 || eml.emailstatus == 2"><i class="fa-solid fa-envelope-open-text text-dark"></i></a>
                                <a type="button" ng-click="email_delete(eml.enid)" ng-hide="eml.emailstatus == 1 || eml.emailstatus == 2"><i class="fa-regular fa-trash text-danger"></i></a>
                            </div>
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
        <div class="d-flex align-items-center justify-content-end pt-3">
            <ul style="margin-bottom: 0 !important;" uib-pagination total-items="filteredi.length" num-pages="numPages" items-per-page="items_per_page" ng-model="current_page" max-size="5" boundary-link-numbers="true" ng-change="pageChanged()"></ul>
        </div>
    </div>
</div>
<?php include "../print/emn_print.php" ?>